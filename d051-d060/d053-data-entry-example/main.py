...
# Create a bot that scraps info from Zillow (a clone, actually), and
# then fill the info into a Google form.

# Again, Google account is needed, and there is no reason to open one
# just for the project. So the code below is a copy from the curse
# solution.
...
all_address_elements = soup.select(
    ".StyledPropertyCardDataWrapper address")
all_addresses = [
    address.get_text().replace(" | ", " ").strip()
    for address in all_address_elements]
print(
    f"\n After having been cleaned up, "
    f"the {len(all_addresses)} addresses now look like this: \n")
print(all_addresses)
...
print(
    f"\n After having been cleaned up, "
    f"the {len(all_prices)} prices now look like this: \n")
print(all_prices)
...
for n in range(len(all_links)):
    ...
    address = driver.find_element(
        by=By.XPATH,
        value='//*[@id="mG61Hd"]/div[2]/'
              'div/div[2]/div[1]/div/div/div[2]/'
